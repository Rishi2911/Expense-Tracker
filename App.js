  ...
  item.querySelector("#edit").addEventListener("click", () => {
    ...
    if (flag) {
      ...
      if (editedAmount < budgetNumber + expenseNumber) {
        parentElement.querySelector(".list-expense-title").innerText =
          editedTitle;
        parentElement.querySelector(".list-expense-desc").innerText =
          editedDesc;
  ...
  if (budget.innerText === "Rs 00,000.00") alert("Please Add Budget First!");
    ...
    if (
      ...
      if (budgetNumber >= expenseAmount.value) {
        ...
        createExpenseList(
          arrOfItems[index].title,
          arrOfItems[index].desc,
          arrOfItems[index].amount,
          arrOfItems[index].date
        );
...
const updateBudget = () => {
  if (budgetNumber >= expenseAmount.value) {
    ...
    let currentExpenceNumber = expArray.reduce((a, b) => {
      return a + b;
    });
